...
def expected_op(op, wf):
    ...
    """
    return np.matmul(np.conj(wf), np.matmul(op, wf)).real
...
def normalize(wf):
    """
    Normalize given wavefunction

    Args:
        wf (np array): numpy array that represents a wavefunction
    """
    return (1./np.linalg.norm(wf))*wf
