...
if rerun == True:
    for nqbts in range (4, 12): 
        ...
        y_axis_start = 10**-4
        y_axis_end = .2
        p2_arr = np.linspace(y_axis_start, y_axis_end, 50)
        for i, p2 in enumerate(p2_arr):
            ...
            if os.path.exists(p2_str) == False:
                ...
                # if os.path.exists("HR_dist_hist.pkl") == True and remove_measurement == True:
                #     p = subprocess.Popen(["rm", "HR_dist_hist.pkl"])
                #     p.wait()
            ...
            with open(os.path.join(qubit_dir,p2_str + "/fid_hist.pkl"), "rb") as Fid_file:
                Fid_collection[nqbts-4, i] = np.average(np.array(pickle.load(Fid_file))**2)
                print(Fid_collection[nqbts-4, i])
            Fid_file.close()
            # run_str = ["python3.9", "leo_HR.py", "--input_dir", str(os.path.join(qubit_dir,value)), "--p1", p1, "--p2", p2] 

            # p = subprocess.Popen(run_str)
            # p.wait()

            # with open(os.path.join(qubit_dir,value + "/HR_dist_hist_2.pkl"), "rb") as HR_file:
            #     dict["HR_dist"] = np.average(pickle.load(HR_file))
            # HR_file.close()
            # dict_arr.append(dict)
            # np.save("HR_testing.npy", dict_arr)
        slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(Fid_collection[nqbts-4], HR_collection[nqbts-4])
...
plt.savefig("correlations_ALA.png",dpi = 300, bbox_inches='tight')
plt.clf()
