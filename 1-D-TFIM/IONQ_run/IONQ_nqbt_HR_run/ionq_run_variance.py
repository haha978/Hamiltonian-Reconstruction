...
def main(args):
    ...
    for param_idx in param_idx_l:
        ...
        print("This is vairance: ", var_H)
        var_hist.append(var_H)
        with open(os.path.join(args.input_dir, "var_hist.pkl"), "wb") as fp:
    ...
    plt.savefig(args.input_dir+'/'+  str(n_qbts)+"qubits_"+ str(n_layers)+f"layers_shots_{shots}_variance.png", dpi = 300, bbox_inches='tight')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = "Hamiltonian Variance for 1-D TFIM with non-periodic boundary condition")
